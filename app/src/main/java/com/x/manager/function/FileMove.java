...
public class FileMove {
    ...
    public static void run(List<FileListItem> selFiles, File destFolder) {
        ...
        SanctionBase.LogNormal(MainActivity.Self, "Move initialization started.");
        new Thread(() -> {
            fileCheck(++fileIndex);
        }).start();
    }
    ...
    public static void handleResult(int resultCode, Intent intent) {
        if (resultCode != RESULT_OK) {
            SanctionBase.LogNormal(MainActivity.Self, "Move operation cancelled - dialog cancel.");
            cancelOperation();
            return;
        }
        ...
        if (selOp == PasteOptionsDialog.OPTION_ABORT) {
            SanctionBase.LogNormal(MainActivity.Self, "Move operation cancelled - user abort.");
            cancelOperation();
            return;
        }
    ...
    private static void startOperation() {
        SanctionBase.LogNormal(MainActivity.Self, "Move operation started.");
        ...
        intent.putExtra(ProgressDialog.PARAM_TITLE, "Move");
        ...
        for (int i = 0; i < files.size(); ++i) {
            if (ProgressDialog.ProgressCancelled) {
                ...
                SanctionBase.LogNormal(MainActivity.Self, "Move operation cancelled - progress cancel.");
                cancelOperation();
                return;
            }
            ...
            ProgressDialog.ChangeProgress("Moving " + i + " of " + files.size() + ": "
                    + "\n" + oldPath, true, true);
            ...
            if (options.get(i) == PasteOptionsDialog.OPTION_NONE) {
                outFile = new File(newPath + fName);
                res = Tools.RenameFile(f, outFile);
            } else if (options.get(i) == PasteOptionsDialog.OPTION_COPY) {
                outFile = new File(Tools.GenerateCopyName(newPath, fName));
                res = Tools.RenameFile(f, outFile);
                ...
                if (outFile.exists()) {
                    if (f.isFile() && outFile.isFile() && options.get(i) == PasteOptionsDialog.OPTION_OVERWRITE) {
                        res = FileDelete.deleteFile(MainActivity.Self, outFile);
                        if (res) {
                            res = Tools.RenameFile(f, outFile);
                        }
                    } else if (f.isDirectory() && outFile.isDirectory() && options.get(i) == PasteOptionsDialog.OPTION_MERGE) {
                        res = Tools.CopyFile(f, new File(newPath), Tools.CopyMode.FolderMerge);
                        if (res) {
                            res = FileDelete.deleteFileTree(MainActivity.Self, f);
                        }
                    } else {
                ...
                } else { // ignore file check, file no longer exists so do normal move
                    res = Tools.RenameFile(f, outFile);
                }
            }
        ...
        SanctionBase.LogNormal(MainActivity.Self, "Move operation completed.");
...
//        }
        ...
        String oldPath = f.getAbsolutePath();
        if (f.isDirectory()) {
            oldPath = Tools.DeletePathTrail(oldPath);
        }
        ...
        if (newFile.exists()) {
            ...
            if (remOption != PasteOptionsDialog.OPTION_NONE
                ...
                MainActivity.Self.startActivityForResult(intent, MainActivity.REQUEST_PASTE_OPTION_MOVE);
                MainActivity.Self.overridePendingTransition(R.anim.slide_up, R.anim.slide_down);
                return;
            }
        } else {
